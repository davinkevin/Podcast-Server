...
export class SearchComponent implements OnInit, OnDestroy {
  ...
  properties = [
    {title: 'Relevance', value: 'pertinence'}, {title: 'Publication Date', value: 'pubDate'},
    {title: 'Download Date', value: 'downloadDate'}
  ];

  directions =
      [{title: 'Descendant', value: Direction.DESC}, {title: 'Ascendant', value: Direction.ASC}];
  ...
  ngOnInit() {
    ...
    this.form.valueChanges.pipe(untilDestroy(), debounceTime(500), map(toSearchItemRequest))
  ...
  search(v: SearchItemPageRequest): void {this.store.dispatch(new Search({...defaultSearch, ...v}))}
...
function toStatus(v: StatusesViewValue): Status[] {
  switch (v) {
    ...
    default:
      return [
        Status.NOT_DOWNLOADED, Status.DELETED, Status.STARTED, Status.STOPPED, Status.PAUSED,
        Status.FAILED
      ];
  }
}
...
function toSearchItemRequest(v: SearchItemRequestViewModel): SearchItemPageRequest {
  return {
    ...v, status: toStatus(v.status), tags: v.tags.split(',').map(t => ({id: t, name: t})),
        sort: [v.sort]
  }
}
