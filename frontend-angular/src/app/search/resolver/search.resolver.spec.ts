...
describe('SearchResolver', () => {
  ...
  it('should trigger search with current page parameter', inject([SearchResolver], (resolver: SearchResolver) => {
    ...
    resolver.resolve(route, null).subscribe(v => {
      /* Then  */
      expect(v).toBe(PAGE);
      expect(itemService.search).toHaveBeenCalledWith({ page: NUM_PAGE, size: 12, downloaded: true, tags: [], sort: [{ property: 'pubDate', direction: 'DESC' }]});
    })
  }));
});
