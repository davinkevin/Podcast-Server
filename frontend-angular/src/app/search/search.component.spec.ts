...
describe('SearchFeature', () => {
	describe('SearchComponent', () => {
		...
		describe('with simple request and result', () => {
			...
			it(
				'should generate a search when q form is changed',
				fakeAsync(() => {
					/* Given */
					const q = el.query(By.css('input[name="q"]'));
			...
			it(
				'should generate a search when tags form is changed',
				fakeAsync(() => {
					/* Given */
					const tags = el.query(By.css('input[name="tags"]'));
			...
			it(
				'should map to FINISH status if DOWNLOADED is selected in the UI',
				fakeAsync(() => {
					/* Given */
			...
			it(
				'should map to others status if NOT_DOWNLOADED is selected in the UI',
				fakeAsync(() => {
					/* Given */
					const statusForm = comp.form.get('status');
			...
			it(
				'should change page and make request if next is clicked',
				fakeAsync(() => {
					/* Given */
		...
		describe('with special request', () => {
			...
			describe('with Statuses', () => {
				describe('with FINISH status', () => {
					...
					it(
						'should be initialized with tags list',
						async(() => {
							fixture.whenStable().then(() => {
								const status = el.query(By.css('mat-select[name="status"]'));
