...
class DailymotionExtractor(val json: JsonService, val html: HtmlService, val m3u8: M3U8Service) : Extractor {
    ...
    override fun extract(item: DownloadingItem): DownloadingInformation {
        ...
        val urlAsString = item.url.toASCIIString()

        return html.get(urlAsString)
                ...
                .map { it.url.replace(":videoId", toId(urlAsString)) }
                .flatMap { json.parseUrl(it) }
                .map { JsonService.to(DailymotionMetadataItemExtractor::class.java).apply(it) }
                .map { it.url }
                .map { m3u8.getM3U8UrlFormMultiStreamFile(it) }
                .map { it.substringBefore("#") }
                .map { it to getFileName(item.url) }
                .map { DownloadingInformation(item, listOf(it.first), it.second, null) }
    ...
    override fun compatibility(url: URI): Int =
            if ("dailymotion.com/video" in url.toASCIIString() ) 1
