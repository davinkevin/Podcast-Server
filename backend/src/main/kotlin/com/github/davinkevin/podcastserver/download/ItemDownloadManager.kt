...
class ItemDownloadManager (
    ...
    private fun launchDownloadFor(item: DownloadingItem) {
        log.debug("inside launchDownloadFor")
...
class QueueProxy(
    ...
    fun removeItemFromQueue(id: UUID, stopItem: Boolean): Mono<Void> {
        return Mono.zip(
            removeItemFromQueue(id),
            if (stopItem) repository.stopItem(id).then() else Mono.empty()
        )
            .then()
    }

    fun removeACurrentDownload(id: UUID): Mono<Void> {
        val pairs = downloadingQueue
            .map { it.toPair() }
            .filter { it.first.id != id }
            .toTypedArray()

        downloadingQueue = mapOf(*pairs)
