...
//            val podcast1 = PodcastToUpdate(id = UUID.randomUUID(), url = URI("http://foo.bar.com/foo1.xml"), signature = "" )
//            val podcast2 = PodcastToUpdate(id = UUID.randomUUID(), url = URI("http://foo.bar.com/foo2.xml"), signature = "" )
//            val podcast3 = PodcastToUpdate(id = UUID.randomUUID(), url = URI("http://foo.bar.com/foo3.xml"), signature = "" )
...
//            doAnswer { val p = it.getArgument<Podcast>(0); UpdatePodcastInformation(p, generateItems(10, p), "foo") }.whenever(updater).update(podcast3)
//            doAnswer { val p = it.getArgument<Podcast>(0); UpdatePodcastInformation(p, p.items!!, "foo") }.whenever(updater).update(argWhere { it != podcast3 })
...
//            val now = ZonedDateTime.now()
//            val uuid = UUID.randomUUID()
//            val podcast = PodcastToUpdate(id = uuid, url = URI("http://foo.bar.com/foo1.xml"), signature = "")
//            val updater = mock<Updater>()
//            val items = generateItems(10, podcast).toVΛVΓ()
//            whenever(podcastRepository.findById(uuid)).thenReturn(Optional.of(Podcast().apply { id = uuid; title = "podcast"; url = "http://foo.bar.com/foo1.xml" }))
//            whenever(updaterSelector.of(podcast.url)).thenReturn(updater)
//            whenever(updater.update(any())).then {
//                val p = it.getArgument<PodcastToUpdate>(0)
//                UpdatePodcastInformation(p, items.toJavaSet(), "foo")
//            }
//            whenever(validator.validate(any<Item>())).thenReturn(setOf<ConstraintViolation<Item>>())
//
//            /* When */
...
//    }
    ...
    private fun generateItems(number: Int, p: PodcastToUpdate): Set<Item> {
        return (1..number)
                .map { Item().apply {
