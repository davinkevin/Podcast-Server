 ...
 */
    ...
    var UpdatingDirective = function () {
        function UpdatingDirective() {
            _classCallCheck(this, UpdatingDirective);
            this.restrict = 'E';
            this.scope = true;
        ...
        _createClass(UpdatingDirective, [{
                key: 'link',
                value: function link(scope, element) {
                    var liParent = element.parent().parent()[0];
                    if (liParent && liParent.tagName === 'LI') {
                        (function () {
                            var liElement = angular.element(liParent);
                            liElement.addClass('hidden');
                            scope.$watch('uc.isUpdating', function (newValue) {
                                return newValue ? liElement.removeClass('hidden') : liElement.addClass('hidden');
                            });
                        }());
                    }
                }
            }]);
        return UpdatingDirective;
    }();
    var UpdatingController = function () {
        function UpdatingController(ngstomp, $scope) {
            var _this2 = this;
            _classCallCheck(this, UpdatingController);
            this.ngstomp = ngstomp;
            this.isUpdating = false;
            this.$scope = $scope;
            this.ngstomp.subscribe('/app/updating', function (message) {
                return _this2.updateStatus(message);
            }, $scope).subscribe('/topic/updating', function (message) {
                return _this2.updateStatus(message);
            }, $scope);
        }
        UpdatingController.$inject = ["ngstomp", "$scope"];
        _createClass(UpdatingController, [{
                key: 'updateStatus',
    ...
    angular.module('updating', ['AngularStompDK']).directive('updateStatus', function () {
        return new UpdatingDirective();
 ...
 */
    var itemService = function () {
        ...
        _createClass(itemService, [
            ...
            },
            {
                ...
                value: function restangularizedItems(itemList) {
                    ...
                    angular.forEach(itemList, function (value) {
                        restangularList.push(_this16.Restangular.restangularizeElement(_this16.Restangular.one('podcast', value.podcastId), value, _this16.childRoute));
                    });
            ...
            {
                key: 'upload',
                value: function upload(podcast, file) {
                    var formData = new FormData();
                    formData.append('file', file);
                    return podcast.all(this.childRoute).withHttpConfig({ transformRequest: angular.identity }).customPOST(formData, 'upload', undefined, { 'Content-Type': undefined });
                }
            }
        ]);
        return itemService;
 ...
 */
    ...
    var podcastUploadDirective = function podcastUploadDirective() {
        _classCallCheck(this, podcastUploadDirective);
        this.restrcit = 'E';
    ...
    var podcastUploadCtrl = function () {
        function podcastUploadCtrl($scope, itemService, $notification) {
            _classCallCheck(this, podcastUploadCtrl);
            this.$scope = $scope;
            this.itemService = itemService;
            this.$notification = $notification;
        }
        podcastUploadCtrl.$inject = ["$scope", "itemService", "$notification"];
        _createClass(podcastUploadCtrl, [{
                key: 'onFileSelect',
                value: function onFileSelect($files) {
                    var _this27 = this;
                    angular.forEach($files, function (file) {
                        _this27.itemService.upload(_this27.podcast, file).then(function (item) {
                            _this27.$scope.$emit('podcastEdition:upload');
                            try {
                                _this27.$notification('Upload effectu\xE9', {
                                    body: item.title,
                                    icon: item.cover.url,
                                    delay: 5000
                                });
    ...
    angular.module('ps.podcast.details.upload', [
        ...
        return new podcastUploadDirective();
